...
function renderNetworkError(message) {
  if (!networkErrorEl) {
    networkErrorEl = document.createElement('div');
    networkErrorEl.classList.add('network-error');
    document.body.appendChild(networkErrorEl);
  }
  networkErrorEl.textContent = message;
...
async function pingServer() {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      console.warn("Запрос прерван: Медленное соединение");
    } else {
      renderNetworkError("Проблема с сетью");
    }
  } finally {
